 ...
 *
 * SERVER ONLY. These functions read `LOVABLE_API_KEY` from the server
 * environment and talk to the Lovable connector gateway directly. Importing
 * this module from a browser bundle would leak workspace credentials, so it is
 * only ever imported from `createServerFn` handlers and server routes.
 */
...
export interface AppUserOAuthAuthorizeParams {
  ...
  clientAPIKey: string;
  returnUrl: string;
  /** Present on reconnect only, so the gateway can confirm ownership. */
...
/** A 401 whose body type starts with "credential_" means: reconnect needed. */
...
/** Swaps the one-time redirect `code` for the per-user connection key. */
